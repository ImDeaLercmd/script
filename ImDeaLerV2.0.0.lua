...
local UPDATE_COOLDOWN = 1800 -- Время между проверками (в секундах)

-- Переменные для системы обновления
...
function checkForUpdates()
    if not CHECK_UPDATE then
        print("🔕 Проверка обновлений отключена")
        return
    end
    ...
    -- Проверяем кулдаун
    if currentTime - lastUpdateCheck < UPDATE_COOLDOWN then
        print("⏳ Следующая проверка обновлений через " .. (UPDATE_COOLDOWN - (currentTime - lastUpdateCheck)) .. " сек")
        return
    end
    ...
    if response and response.code == 200 then
        ...
        local remoteContent = response.content
        
        -- Ищем строку с версией (разные варианты)
        local patterns = {
            "local VERSION = \"([%d%.]+)\"",
            "local VERSION%s*=%s*\"([%d%.]+)\"",
            "VERSION%s*=%s*\"([%d%.]+)\"",
        ...
        for _, pattern in ipairs(patterns) do
            remoteVersion = remoteContent:match(pattern)
            if remoteVersion then break end
        end
        
        if remoteVersion then
            print("📊 Локальная версия: " .. VERSION)
            print("📊 Удаленная версия: " .. remoteVersion)
            
            -- Сравниваем версии
            if compareVersions(remoteVersion, VERSION) > 0 then
                updateAvailable = true
                print("📥 Доступно обновление: " .. remoteVersion)
                gg.toast("📥 Доступно обновление v" .. remoteVersion)
                
                -- Показываем уведомление
                showUpdateNotification(remoteVersion, remoteContent)
            else
            ...
            print("⚠️ Не удалось определить версию в удаленном файле")
            gg.toast("⚠️ Ошибка проверки версии")
        end
    else
        print("❌ Ошибка подключения к GitHub")
        if response then
            print("Код ошибки: " .. response.code)
        end
        gg.toast("❌ Ошибка соединения с GitHub")
    end
end
...
function showUpdateNotification(newVersion, newContent)
    gg.setVisible(false)
    gg.sleep(100)
    
    local choice = gg.choice({
        "✅ Да, обновить до версии " .. newVersion,
        "⏰ Напомнить позже",
        "🚫 Пропустить это обновление",
        "ℹ️ Показать информацию об обновлении"
    }, nil, "🎉 ДОСТУПНО ОБНОВЛЕНИЕ!\n\n" ..
           "Текущая версия: " .. VERSION .. "\n" ..
           "Новая версия: " .. newVersion .. "\n\n" ..
           "Хотите обновить скрипт?")
    
    if choice == 1 then
        -- Сохраняем обновленный скрипт
        local scriptPath = gg.EXT_STORAGE .. "/GameGuardian/scripts/" .. SCRIPT_NAME
        ...
        if file then
            ...
            print("📁 Путь: " .. scriptPath)
            
            gg.alert("✅ СКРИПТ ОБНОВЛЕН!\n\n" ..
                    "Старая версия: " .. VERSION .. "\n" ..
                    "Новая версия: " .. newVersion .. "\n\n" ..
                    "Перезапустите скрипт для применения изменений")
            ...
            gg.alert("❌ Ошибка сохранения обновления!\nПроверьте права доступа.")
        end
    elseif choice == 2 then
        print("⏰ Напоминание отложено")
        gg.toast("⏰ Напоминание через " .. UPDATE_COOLDOWN .. " сек")
    elseif choice == 3 then
        print("🚫 Обновление пропущено")
        gg.toast("🚫 Обновление пропущено")
        -- Можно сохранить версию в настройках, чтобы не предлагать снова
    elseif choice == 4 then
        showUpdateInfo(newVersion, newContent)
    end
end

-- Функция показа информации об обновлении
function showUpdateInfo(newVersion, newContent)
    -- Извлекаем информацию об изменениях
    local changelog = "Изменения в версии " .. newVersion .. ":\n\n"
    
    -- Ищем changelog в скрипте (разные форматы)
    local patterns = {
        "--%s*Изменения:?%s*\n(.-)\n\n",
        "--%s*CHANGELOG:?%s*\n(.-)\n\n",
        "--%s*Обновление.*\n(.-)\n\n",
    ...
    local foundChangelog = false
    for _, pattern in ipairs(patterns) do
        local log = newContent:match(pattern)
        if log then
    ...
    if not foundChangelog then
        changelog = changelog .. "Изменения не указаны.\nРекомендуется обновить скрипт."
    end
    
    local choice = gg.choice({
        "⬆️ Обновить сейчас",
        "⬇️ Скачать, но не устанавливать",
        "📋 Копировать ссылку на GitHub",
        "↩️ Назад к обновлению"
    }, nil, changelog .. "\n\nРазмер: " .. #newContent .. " байт")
    ...
    elseif choice == 2 then
        ...
        local newScriptName = "ImDeaLerV" .. newVersion .. ".lua"
        local newPath = gg.EXT_STORAGE .. "/GameGuardian/scripts/" .. newScriptName
        local file = io.open(newPath, "w")
        if file then
            file:write(newContent)
            file:close()
            gg.alert("✅ Скачано как: " .. newScriptName .. "\n\nПуть: " .. newPath)
        end
    elseif choice == 3 then
        -- Копировать ссылку
        gg.copyText(GITHUB_URL)
        gg.toast("✅ Ссылка скопирована в буфер")
    elseif choice == 4 then
        -- Назад
        showUpdateNotification(newVersion, newContent)
    end
end

-- Функция создания backup
function createBackup()
    local backupName = "ImDeaLerV" .. VERSION .. "_backup.lua"
    local backupPath = gg.EXT_STORAGE .. "/GameGuardian/scripts/backups/" .. backupName
    
    -- Создаем папку для backup если ее нет
    local backupDir = gg.EXT_STORAGE .. "/GameGuardian/scripts/backups"
    os.execute("mkdir -p \"" .. backupDir .. "\"")
    
    -- Копируем текущий скрипт
    local currentPath = gg.EXT_STORAGE .. "/GameGuardian/scripts/" .. SCRIPT_NAME
    os.execute("cp \"" .. currentPath .. "\" \"" .. backupPath .. "\"")
    
    print("💾 Backup создан: " .. backupName)
end

-- Функция принудительной проверки обновлений
function forceCheckUpdate()
    print("⚡ Принудительная проверка обновлений...")
    lastUpdateCheck = 0 -- Сбрасываем таймер
    checkForUpdates()
end
...
function forceUpdate()
    ...
    if response and response.code == 200 then
        ...
        if file then
            ...
            gg.alert("❌ Ошибка сохранения файла")
        end
    else
        gg.alert("❌ Ошибка загрузки обновления")
    end
end
...
function checkInternet()
    ...
    for _, site in ipairs(testSites) do
        local response = gg.makeRequest(site)
        if response and response.code == 200 then
            print("✅ Соединение с " .. site .. " установлено")
            return true
        end
    end
...
-- Функция для отображения информации о версии
function showVersionInfo()
    local info = "📱 ImDeaLer Script\n\n" ..
                "Версия: " .. VERSION .. "\n" ..
                "Автообновление: " .. (CHECK_UPDATE and "ВКЛ" or "ВЫКЛ") .. "\n" ..
                "GitHub: " .. GITHUB_URL .. "\n\n" ..
                "Последняя проверка: " .. os.date("%H:%M:%S", lastUpdateCheck)
    
    if updateAvailable and remoteVersion then
        info = info .. "\n\n⚠️ ДОСТУПНО ОБНОВЛЕНИЕ: v" .. remoteVersion
    end
    ...
    if choice == 1 then
        forceCheckUpdate()
    elseif choice == 2 then
        local confirm = gg.alert("⚠️ ПРИНУДИТЕЛЬНОЕ ОБНОВЛЕНИЕ\n\n" ..
                                "Текущая версия будет заменена\n" ..
                                "на последнюю из GitHub.\n\n" ..
                                "Продолжить?", "✅ Да", "❌ Нет")
        if confirm == 1 then
            forceUpdate()
        end
    elseif choice == 3 then
        if checkInternet() then
            gg.toast("✅ Интернет соединение установлено")
        else
            gg.alert("❌ Нет подключения к интернету")
        end
    elseif choice == 4 then
        local copyText = "ImDeaLer Script v" .. VERSION .. "\n" ..
                        "GitHub: " .. GITHUB_URL
        gg.copyText(copyText)
        gg.toast("✅ Информация скопирована")
    end
end

-- Добавляем пункт обновлений в главное меню
local originalMainMenu = nil

-- Перехватываем главное меню
function setupAutoUpdateMenu()
    -- Сохраняем оригинальное меню
    if not originalMainMenu then
        originalMainMenu = t('mainMenu')
    end
    
    -- Создаем новое меню с пунктом обновлений
    local newMainMenu = {}
    for i, item in ipairs(originalMainMenu) do
        newMainMenu[i] = item
    end
    
    -- Добавляем пункт обновлений перед последним пунктом (выход)
    table.insert(newMainMenu, #newMainMenu, "🔄 Система обновлений")
    
    -- Обновляем перевод
    translations[currentLanguage].mainMenu = newMainMenu
end

-- Модифицируем функцию mainch для обработки нового пункта
local originalMainCh = mainch
function mainch()
    setupAutoUpdateMenu()
    
    local result = originalMainCh()
    
    -- Проверяем выбран ли пункт обновлений
    local menuItems = t('mainMenu')
    for i, item in ipairs(menuItems) do
        if item:find("🔄 Система обновлений") and men == i then
            showVersionInfo()
            return "continue"
        end
    end
